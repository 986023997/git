...
<body>
	...
	<span class="c-gray en">&gt;</span> 管理员 <span class="c-gray en">&gt;</span>
	用户列表<a class="btn btn-success radius r mr-20"
	...
	<div>
	<a href="javascript:;" onclick="paper_edit('添加账号','<%=basePath%>AddUserUIServlet','','510')" 
				class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加账号</a>
		...
		<div class="mt-20">
			...
			<table
				...
				<c:forEach items="${users}" var="que" varStatus="status">
					<tbody>
						<tr class="text-c">
							<td>${status.index+1 }</td>
							<td>${que.user_name}</td>
							<td>${que.user_realname}</td>
							<td>${que.user_sex}</td>
							<td>${que.user_age}</td>
							<td>${que.user_phone}</td>
							<td>${que.user_mail}</td>
							<td>${que.user_address}</td>
							<td>${que.user_date}</td>
					<!-- 是否替换为课程和章节 -->
							<td class="f-14">
								...
								<a title="修改" href="AdminUpdateUserUIServlet?Id=${que.user_id}" ><i class="Hui-iconfont" onclick="return confirmd()" >修改</i></a>
								&nbsp;&nbsp;&nbsp;&nbsp;
								<a title="删除" href="DeleteUserServlet?Id=${que.user_id}" ><i class="Hui-iconfont" onclick="return confirmd()" >删除</i></a>
							</td>
						</tr>
					</tbody>
				</c:forEach>
